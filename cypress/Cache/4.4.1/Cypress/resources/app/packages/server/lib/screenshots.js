(function() {
  var Jimp, Promise, RUNNABLE_SEPARATOR, _, __ID__, captureAndCheck, clearMultipartState, colorString, crop, dataUriToBuffer, debug, ensureUniquePath, fs, getBuffer, getDimensions, getPath, getPathToScreenshot, getType, glob, hasHelperPixels, imagesMatch, intToRGBA, isAppOnly, isBlack, isMultipart, isWhite, lastImagesAreDifferent, mime, multipartConditionFn, multipartImages, path, pathHelpers, pathSeparatorRe, pixelConditionFn, plugins, sanitize, sanitizeToString, sizeOf, stitchScreenshots,
    slice = [].slice;
  ...
  sanitizeToString = function(title) {
  ...
  getPath = function(data, ext, screenshotsFolder) {
    ...
    if (data.name) {
      ...
      names = _.chain(data.titles).map(sanitizeToString).join(RUNNABLE_SEPARATOR).concat([]).value();
    }
